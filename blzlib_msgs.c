...
static int parse_msg_interfaces_dev(sd_bus_message* m, const char* opath, blz* ctx)
{
	int r;
	const char* str;
	...
	while ((r = sd_bus_message_enter_container(m, 'e', "sa{sv}")) > 0)
	{
		...
		if (strcmp(str, "org.bluez.Device1") == 0) {
			r = parse_msg_device_properties(m, opath, ctx);
		}
		else {
			r = sd_bus_message_skip(m, "a{sv}");
			if (r < 0) {
...
int parse_msg_objects_dev(sd_bus_message* m, blz* ctx)
{
	...
	while ((r = sd_bus_message_enter_container(m, 'e', "oa{sa{sv}}")) > 0)
	{
		...
		/* check if it is below our own device path, otherwise skip */
		if (strncmp(opath, ctx->path, strlen(ctx->path)) == 0) {
			r = parse_msg_interfaces_dev(m, opath, ctx);
			if (r == 1000)
				return r;
		} else {
			r = sd_bus_message_skip(m, "a{sa{sv}}");
